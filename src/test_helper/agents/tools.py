...
class InteractionEventJSON(TypedDict):
    type: str
    payload: dict[str, str | None]


async def browser_navigate(url: str) -> InteractionEventJSON:
    """Navigate via MCP and return event as dict."""
    async with MCPClient() as mcp:
        ev = await mcp.navigate(url)
        return cast("InteractionEventJSON", ev.model_dump(mode="json"))
...
async def browser_fill(selector: str, value: str) -> InteractionEventJSON:
...
def emit_patch(project_id: str, spec_rel_path: str, diff: str) -> dict[str, str]:
    ...
    if spec_path.suffixes[-2:] != [".spec", ".ts"]:
